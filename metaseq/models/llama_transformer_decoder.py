...
from apex.normalization.fused_layer_norm import FusedRMSNorm  # type: ignore
...
from metaseq.models.transformer_decoder import (
    ...
    TransformerDecoderMultiLayerBlockModule,
)
from metaseq.modules.transformer_decoder_layer import _weight_init
...
from metaseq.modules.rope import RotaryEmbedding
from metaseq.modules.llama_model_parallel_transformer_decoder_layer import (
    LlamaModelParallelTransformerDecoderLayer,
)
...
class LlamaTransformerDecoder(BaseDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        norm_class = (
            FusedRMSNorm
            if getattr(args, "layernorm_type", "layernorm")
            in ["fusedrmsnorm", "rmsnorm"]
            else LayerNorm
        )
        ...
        if self.share_input_output_embed:
            self.output_projection = self.build_output_layer(
            ...
            self.output_projection = self.build_output_layer(
                self.embed_dim, len(dictionary), **output_layer_args
            )
        ...
        if self.use_rope:
            # rope
            self.rope = RotaryEmbedding(self.embed_dim // args.decoder_attention_heads)
    ...
    def build_output_layer(
        ...
        **unused_args,
        ...
        nn.init.normal_(layer.weight, mean=0, std=self.embed_dim**-0.5)
        return layer

    def build_base_decoder_layer(self, args):
        return LlamaTransformerDecoderLayer(args)
    ...
    def extract_features(
        ...
        if self.use_rope:
            start = 0
            if incremental_state:
                first_key = list(incremental_state.keys())[0]
                first_inner_key = list(incremental_state[first_key].keys())[0]
                first_val = incremental_state[first_key][first_inner_key]
                if first_val is not None:
                    start = first_val.size(-2) if first_val.numel() > 0 else 0
            end = prev_output_tokens.size(-1)
            if start == end:
                end += 1
            freqs_cis = self.rope.get_freqs_cis(start, end)
...
class LlamaModelParallelTransformerDecoder(LlamaTransformerDecoder):
    """
    ...
    def build_base_decoder_layer(self, args, **kwargs):
        return LlamaModelParallelTransformerDecoderLayer(args)

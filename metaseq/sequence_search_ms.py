...
class SequenceSearch(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
    ...
    def _generate(
        ...
        tks = torch.stack([ torch.stack([beam["tokens"] for beam in sent]) for sent in finalized])
        scs = torch.stack([ torch.stack([beam["positional_scores"] for beam in sent]) for sent in finalized])

        retval = {"tokens": tks, "scores": scs}

...
from metaseq.modules.llama_multihead_attention import (
    LlamaMultiheadAttention,
)


class LlamaTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, args, add_bias_kv=False, add_zero_attn=False, **kwargs):
        super().__init__()
        load_megatron_fused_kernel()
        self.args = args
        ...
        if not llama_init_order:
            ...
            self.self_attn_layer_norm = norm_class(
        ...
        self.final_layer_norm = norm_class(self.embed_dim, elementwise_affine=affine_ln)
        self.final_layer_norm.to(device).to(dtype)

        self.args = args
        layer_scale_type = getattr(args, "layer_scale", "disabled")
        self.layer_scale = layer_scale_type != "disabled"
        if self.layer_scale:
            values = {
                "current": kwargs.get("layer_id", 1) + 1,
                "global": args.decoder_layers,
                "one": 1,
            }
            alpha = 1.0 / math.sqrt(values[layer_scale_type])
            self.alpha = nn.Parameter(torch.full((self.embed_dim,), alpha))
            self.alpha.to(device).to(dtype)
    ...
    def build_self_attention(
        ...
        return LlamaMultiheadAttention(
            embed_dim,
